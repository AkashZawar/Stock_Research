...
].map(([symbol, name, tags]) => ({ symbol, name, tags }));

const commodityImpactGroups = [
  {
    commodity: "Oil",
...
const server = http.createServer(async (req, res) => {
  try {
    ...
    if (url.pathname === "/api/market-monitor") {
      await handleMarketMonitor(url, res);
      return;
    }
...
async function handleMarketMonitor(url, res) {
  if (url.searchParams.get("refresh") === "1") {
    cache.delete("market-monitor");
  }
  const data = await cached("market-monitor", buildMarketMonitor, 10 * 60 * 1000);
...
async function buildMarketMonitor() {
  const [commodityResults, scanResults] = await Promise.allSettled([
    settleMap(commodities, getCommoditySnapshot, 3),
    settleMap(breakoutWatchlist, scanWatchlistStock, 4)
  ]);

  const commoditySnapshots = commodityResults.status === "fulfilled"
    ? commodityResults.value.filter((result) => result.status === "fulfilled").map((result) => result.value)
    : [];
  const scannedStocks = scanResults.status === "fulfilled"
    ? scanResults.value.filter((result) => result.status === "fulfilled").map((result) => result.value)
    : [];
  const candidates = scannedStocks
    .filter((stock) => stock.nearAvailableHigh || stock.near52WeekHigh || stock.breakout || stock.breakoutWatch)
    .sort((a, b) => b.score - a.score)
    .slice(0, 18);
  const impacted = await buildCommodityImpact(commoditySnapshots, scannedStocks);

  return {
    generatedAt: new Date().toISOString(),
    source: "Yahoo Finance public endpoints",
    note: "Available-history high uses the history returned by the provider. Confirm all signals on your broker or TradingView before acting.",
    commodities: commoditySnapshots,
    breakoutCandidates: candidates,
    impacted,
    scannedCount: scannedStocks.length
  };
}

async function getCommoditySnapshot(commodity) {
  const chart = await getChartRange(commodity.symbol, "6mo", "1d");
  ...
  return {
    ...
    changePercent: round(safeDivide(latest - previous, previous) * 100),
    oneWeek: roundOrNull(periodReturn(closes, 5)),
    oneMonth: roundOrNull(periodReturn(closes, 21)),
    trend: commodityTrend(closes),
    lastDate: candles[candles.length - 1]?.date || null
  };
}

async function settleMap(items, mapper, concurrency = 4) {
...
async function scanWatchlistStock(stock) {
  const chart = await getChartRange(stock.symbol, "max", "1d");
  const candles = chart.candles;
  if (candles.length < 80) {
    throw new Error(`Not enough data for ${stock.symbol}`);
  }
  ...
  const availableHigh = Math.max(...highs);
  const highIndex = highs.indexOf(availableHigh);
  const high52 = Math.max(...candles.slice(-252).map((candle) => candle.high));
  const prior20High = Math.max(...candles.slice(-21, -1).map((candle) => candle.high));
  const prior55High = Math.max(...candles.slice(-56, -1).map((candle) => candle.high));
  const atrValue = last(atr(candles.slice(-80), 14)) || latest.close * 0.02;
  const avgVolume20 = last(sma(volumes, 20));
  const volumeRatio = avgVolume20 ? latest.volume / avgVolume20 : null;
  const pctBelowAvailableHigh = safeDivide(availableHigh - latest.close, availableHigh) * 100;
  const pctBelow52High = safeDivide(high52 - latest.close, high52) * 100;
  const breakout = latest.close > prior55High && previous.close <= prior55High;
  const breakoutWatch = latest.close >= prior55High * 0.98 || latest.close >= prior20High * 0.99;
  const nearAvailableHigh = pctBelowAvailableHigh >= 0 && pctBelowAvailableHigh <= 3;
  const near52WeekHigh = pctBelow52High >= 0 && pctBelow52High <= 3;
  const momentum = periodReturn(closes, 21);
  const score = Math.round(
    (breakout ? 35 : 0) +
    (breakoutWatch ? 18 : 0) +
    (nearAvailableHigh ? 25 : 0) +
    (near52WeekHigh ? 18 : 0) +
    (volumeRatio && volumeRatio > 1.2 ? 8 : 0) +
    clamp(momentum || 0, -10, 12)
  );
  ...
  return {
    ...
    changePercent: round(safeDivide(latest.close - previous.close, previous.close) * 100),
    availableHigh: round(availableHigh),
    availableHighDate: candles[highIndex]?.date || null,
    high52Week: round(high52),
    pctBelowAvailableHigh: round(pctBelowAvailableHigh),
    pctBelow52WeekHigh: round(pctBelow52High),
    prior20High: round(prior20High),
    prior55High: round(prior55High),
    breakout,
    breakoutWatch,
    nearAvailableHigh,
    near52WeekHigh,
    ...
    oneMonth: roundOrNull(momentum),
    score,
    signal: describeBreakoutSignal({ breakout, breakoutWatch, nearAvailableHigh, near52WeekHigh })
  };
}

async function buildCommodityImpact(commoditySnapshots, scannedStocks) {
  const quotes = new Map(scannedStocks.map((stock) => [
    stock.symbol,
    {
  ...
  try {
    const providerQuotes = await getQuotes(uniqueImpactSymbols());
    for (const [symbol, quote] of providerQuotes) {
      quotes.set(symbol, quote);
    }
  } catch {
    // The app can still render useful impact context from the already-scanned chart data.
  }
  const commodityByCategory = new Map();

  for (const commodity of commoditySnapshots) {
    const existing = commodityByCategory.get(commodity.category);
    if (!existing || Math.abs(commodity.changePercent) > Math.abs(existing.changePercent)) {
      commodityByCategory.set(commodity.category, commodity);
    }
  }

  return commodityImpactGroups
    .map((group) => {
      const commodity = commodityByCategory.get(group.commodity);
      const move = commodity?.changePercent || 0;
      const direction = move > 0.25 ? "up" : move < -0.25 ? "down" : "flat";
      const focusSymbols = direction === "down" ? group.pressuredWhenUp : [...group.beneficiariesWhenUp, ...group.pressuredWhenUp];

      return {
        commodity: group.commodity,
        reference: commodity ? commodity.name : group.commodity,
        changePercent: round(move),
        direction,
        implication: direction === "up" ? group.whenUp : direction === "down" ? group.whenDown : "Move is small today; watch for follow-through.",
        stocks: focusSymbols.map((symbol) => stockImpactSnapshot(symbol, quotes.get(symbol), group, direction))
      };
    })
    .filter((group) => group.stocks.length);
}

function stockImpactSnapshot(symbol, quote, group, direction) {
  const isBeneficiaryWhenUp = group.beneficiariesWhenUp.includes(symbol);
  return {
    symbol,
    name: watchlistName(symbol),
    role: impactRole(isBeneficiaryWhenUp, direction),
    price: roundOrNull(quote?.regularMarketPrice),
    changePercent: roundOrNull(quote?.regularMarketChangePercent)
  };
}

function impactRole(isBeneficiaryWhenUp, direction) {
  if (direction === "up") {
    return isBeneficiaryWhenUp ? "Potential beneficiary as commodity rises" : "Potential pressure as commodity rises";
  }
  if (direction === "down") {
    return isBeneficiaryWhenUp ? "May cool off as commodity falls" : "May benefit as input cost falls";
  }
  return isBeneficiaryWhenUp ? "Watch for positive commodity sensitivity" : "Watch for input-cost sensitivity";
}

function commodityTrend(closes) {
  const oneWeek = periodReturn(closes, 5);
  const oneMonth = periodReturn(closes, 21);
  if (oneWeek > 1 && oneMonth > 2) {
    return "Rising";
  }
  if (oneWeek < -1 && oneMonth < -2) {
    return "Falling";
...
function describeBreakoutSignal(input) {
  if (input.breakout && input.nearAvailableHigh) {
    return "Breakout near available-history high";
  }
  if (input.breakout) {
    return "Fresh breakout";
  }
  if (input.nearAvailableHigh) {
    return "Near available-history high";
  }
  if (input.near52WeekHigh) {
    return "Near 52-week high";
  }
  return "Close to breakout level";
}

function uniqueImpactSymbols() {
  return [...new Set(commodityImpactGroups.flatMap((group) => [
    ...group.beneficiariesWhenUp,
    ...group.pressuredWhenUp
  ]))];
}

function watchlistName(symbol) {
  return breakoutWatchlist.find((stock) => stock.symbol === symbol)?.name || symbol;
}
...
async function getChart(symbol) {
  return getChartRange(symbol, "1y", "1d");
}

async function getChartRange(symbol, range = "1y", interval = "1d") {
  ...
  const rangedEndpoint = `https://query1.finance.yahoo.com/v8/finance/chart/${encodeURIComponent(symbol)}?range=${encodeURIComponent(range)}&interval=${encodeURIComponent(interval)}&includePrePost=false&events=div%2Csplits`;
  const payload = await fetchJson(range === "1y" && interval === "1d" ? endpoint : rangedEndpoint);
...
async function getQuotes(symbols) {
  ...
  const endpoint = `https://query1.finance.yahoo.com/v7/finance/quote?symbols=${encodeURIComponent(uniqueSymbols.join(","))}`;
  const payload = await fetchJson(endpoint);
  const quotes = (payload.quoteResponse && payload.quoteResponse.result) || [];
...
function buildReport(symbol, meta, candles, quote, summary, sec, screener) {
  ...
  const source = buildSourceLabel(sec, screener);
  ...
  const references = buildReferences(symbol, longName, quote, meta);
...
function buildQualityReport(input) {
  ...
  const availableMetrics = metricNames.filter((name) => Number.isFinite(input.fundamentals[name])).length;
  const chartPoints = input.candles.length;
  const levelCount = (input.supportResistance.supportZones || []).length + (input.supportResistance.resistanceZones || []).length;
  const warnings = [];
  const strengths = [];
  let score = 55;
  ...
  if (chartPoints >= 200) {
    score += 15;
    strengths.push(`One-year chart has ${chartPoints} daily candles.`);
  } else {
    score -= 18;
    warnings.push(`Only ${chartPoints} chart candles were available.`);
  }

  if (levelCount >= 3) {
    score += 8;
    strengths.push(`${levelCount} support/resistance zones were detected.`);
  } else {
    score -= 8;
    warnings.push("Few support/resistance zones were detected.");
  }

  if (availableMetrics >= 7) {
    score += 16;
    strengths.push(`${availableMetrics} key fundamental metrics were available.`);
  } else if (availableMetrics >= 4) {
    score += 5;
    warnings.push("Fundamental coverage is partial.");
  } else {
    score -= 18;
    warnings.push("Fundamental coverage is weak for this symbol.");
  }

  if (input.fundamentals.dataSource && input.fundamentals.dataSource.includes("Screener.in")) {
    score -= 4;
    warnings.push("Indian fundamentals use Screener.in summary parsing, so the fields should be cross-checked.");
  }

  if (!input.quote.regularMarketTime) {
    score -= 4;
    warnings.push("Provider did not return an official market timestamp for the quote.");
  }

  if (input.eventRisk.score >= 65) {
    score -= 12;
    warnings.push("Near-term results/event risk can override technical levels.");
  }

  if (Math.abs(input.technical.score - input.fundamental.score) >= 35) {
    score -= 10;
    warnings.push("Technical and fundamental scores strongly disagree.");
  }

  if (Number.isFinite(input.fundamentals.targetMeanPrice)) {
    strengths.push("Analyst target data was available.");
  } else {
    warnings.push("Analyst target data was not available.");
  }
  ...
  return {
    ...
    summary: score >= 75
      ...
      : score >= 55
        ? "Use this as a research starting point and cross-check important fields."
        : "The report has meaningful data gaps and needs manual verification.",
    chartPoints,
    availableFundamentalMetrics: availableMetrics,
...
function buildScenarios(input) {
  const firstResistance = input.supportResistance.resistanceZones?.[0];
  const firstSupport = input.supportResistance.supportZones?.[0];
  const secondSupport = input.supportResistance.supportZones?.[1];
  const breakout = input.researchLevels.breakoutTrigger;
  const invalidation = input.researchLevels.invalidation;
  const volumeNeeded = input.avgVolume20 ? Math.round(input.avgVolume20 * 1.2) : null;
  const eventWarning = input.eventRisk.score >= 65
    ? "Results/event risk is high, so wait for the event reaction before trusting levels."
    : "No high near-term event risk was detected.";

  return {
    stance: input.technical.score >= 65 && input.eventRisk.score < 65
      ? "Trend continuation"
      : input.technical.score >= 45
        ? "Confirmation needed"
        : "Wait for repair",
    bull: {
      title: "Bull case",
      trigger: breakout,
      confirmation: volumeNeeded
        ? `Daily close above breakout with volume above ${volumeNeeded.toLocaleString("en-US")}.`
        : "Daily close above breakout with above-average volume.",
      expectedMove: input.researchLevels.targets,
      reason: firstResistance
        ? `Price must reclaim resistance near ${round(firstResistance.price)}.`
        : "Price must clear the nearest resistance zone."
    },
    base: {
      title: "Base case",
      rangeLow: firstSupport?.price || input.researchLevels.support,
      rangeHigh: firstResistance?.price || input.researchLevels.resistance,
    ...
    bear: {
      ...
      nextSupport: secondSupport?.price || null,
      action: "A close below invalidation weakens the setup and calls for reassessment.",
      reason: firstSupport
        ? `Nearest support is near ${round(firstSupport.price)}.`
        : "Nearest support was not strong enough to rank."
    },
    safeguards: [
...
function buildReferences(symbol, longName, quote, meta) {
  const tvSymbol = toTradingViewSymbol(symbol, quote, meta);
  ...
  if (tvSymbol) {
    ...
    links.push({
      ...
      url: `https://www.tradingview.com/symbols/${tvSymbol.replace(":", "-")}/technicals/`,
  ...
  if (symbol.endsWith(".NS") || symbol.endsWith(".BO")) {
    links.push({
      ...
      url: `https://www.screener.in/company/${encodeURIComponent(baseSymbol)}/`,
      note: "Cross-check Indian fundamentals and announcements."
    });
    links.push({
      label: "Moneycontrol search",
      url: `https://www.moneycontrol.com/search/?search_str=${encodeURIComponent(longName || baseSymbol)}`,
...
function toTradingViewSymbol(symbol, quote, meta) {
  ...
  const exchange = quote.exchange || meta.exchangeName || "";
  const exchangeMap = {
    NMS: "NASDAQ",
    NAS: "NASDAQ",
    NYQ: "NYSE",
  ...
  return tvExchange ? `${tvExchange}:${symbol}` : symbol;
}
